/* Alang is a collection of tools (functions, types, macros) for the INF214
   class.
...
public:
  ...
  void sync_send(Args... args) {
    ...
    while (receives < target_receives)
      received.wait(lock);
  }
...
template <typename... Streamables> void logl(const Streamables &...ss) {
  CRITICAL;
  _log(ss...);
  std::cout << std::endl;
}
template <typename... Streamables> void log(const Streamables &...ss) {
  CRITICAL;
  _log(ss...);
  std::cout << std::flush;
}
...
template <typename F> scope_guard<F> make_scope_guard(F f) { return f; }
...
// many compilers give a warning. With ignore(x) we can supress a warning about
// unused x.
template <typename T> void ignore(const T &t [[maybe_unused]]) {}
...
// sleep for t
template <class T> void sleep(T t) { std::this_thread::sleep_for(t); }
// sleep for t milliseconds
template <class T> void sleep_ms(T t) { sleep(std::chrono::milliseconds(t)); }
...
// milliseconds
template <class F> auto time_ms(F f) {
...
public:
  ~processes() {
    for (std::thread &ts : this->threads)
      ts.join();
  }
...
bool lock_many(ForwardIterator begin, ForwardIterator end) {
  auto current = begin;
  while (current != end) {
    if (current->first->lock.try_lock()) {
      ++current;
    } else {
...
public:
  template <typename T>
  storeable_t(T x) : self(std::make_shared<model<T>>(std::move(x))) {}
...
private:
  ...
  public:
    ...
    void write_to(ABase *a) const { static_cast<A<T> *>(a)->data = this->data; }
...
struct transaction {
  ...
  transaction(const transaction &) =
      delete; // making sure transactions not copyable
  ...
  void try_commit() {
    ...
    if (write_version >
        this->read_version +
            1) { // no need for read validation, because
      ...
      for (ABase *r : this->read_log) {
        if (this->write_log.count(r) == 1) { // tests if r in write log
          if (r->version <=
              this->read_version) { // no need to lock, because we already
            // hold the lock (bcs r in write log)
            continue;
          }
          // otherwise jump to release_many
        } else if (r->lock.try_lock()) {
          int v = r->version;
          r->lock.unlock();
          if (v <= this->read_version)
            continue;
        } else {
          // this means rollback
        }
    ...
    for (auto e : this->write_log) {
      e.second.write_to(e.first); // e.first is the memory location, e.second
...
void ATOMIC(std::function<void()> code) {
  if (!current_transaction.active)
    ...
    code(); // if nested ATOMIC, just stay in the same transaction
}
...
public:
  ...
  A(const A<T> &t [[maybe_unused]]) : A(this->operator T()) {}
  ...
  operator T() const {
    ...
    if (current_transaction.write_log.count(const_cast<A<T> *>(this)) == 1) {
      return current_transaction.write_log.at(const_cast<A<T> *>(this))
          .template read<T>();
    }
    if (!(this->lock.try_lock()))
      throw roll_back();

    // version check must be while owning lock because version/data are not read
    // atomically
  ...
  void operator=(T u) {
    if (!current_transaction.active) {
      ...
      // In C++20 we could do the following. Currently commented out since not
      // all compilers support it yet
      // current_transaction.write_log.insert_or_assign(const_cast<A<T>*>(this),
      // u);

      // the workaraound in C++17 are the following two lines
      auto r =
          current_transaction.write_log.insert({const_cast<A<T> *>(this), u});
      if (!r.second)
        (*(r.first)).second = u;
    }
  }
...
void execute_concurrently(const std::vector<std::function<void()>> &fs) {
  std::vector<std::thread> threads;
  for (int i : iota_randomized(fs.size()))
...
void collect_tasks(std::vector<std::function<void()>> &tasks [[maybe_unused]]) {
}

template <typename F, typename... Fs>
void collect_tasks(std::vector<std::function<void()>> &tasks, const F &f,
                   const Fs &...fs) {
  tasks.push_back(f);
...
template <typename... Fs> void CO(Fs... fs) {
  std::vector<std::function<void()>> tasks;
  collect_tasks(tasks, fs...);
  execute_concurrently(tasks);
}

void CO(std::vector<std::function<void()>> tasks) {
  execute_concurrently(tasks);
}

template <typename F> void CO(range r, F f) {
  std::vector<std::thread> threads;
  for (int i : r)
...
template <typename... Fs> void IF(Fs... fs) {
...
template <typename... Fs> void DO(Fs... fs) {
  std::vector<std::function<bool()>> conditions;
  std::vector<std::function<void()>> tasks;
  collect_conditional_tasks(conditions, tasks, fs...);
